...
$(function() {
    ...
    $.fn.elastic_grid = function(config){
        ...
        //initial filter nav
        container.html('<div class="wagwep-container center-block" style="text-align: center;"><nav id="porfolio-nav" class="clearfix"><ul id="portfolio-filter" class="nav clearfix"></ul></nav></div>');
        ...
        for (itemIdx = 0; itemIdx < numOfItems; itemIdx++) {
            if(config.items[itemIdx] != undefined){
                ...
                //initial a object
                aObject = $('<a></a>');
                aObject.attr('href', item.large[0]);
                aObject.attr('data-title', item.description);
                aObject.attr('data-lightbox', 'example-set');
...
***************************************************************************/
    ...
    function createList(text){
        var filter = "All"; //text; //.toLowerCase().replace(' ','-');
        // This is a helper function that takes the
        // text of a menu button and array of li items
        if(text != ''){
            var li = $('<li>');
            var a = $('<a>',{
                html: (text != undefined ? text : "All"),
                'data-filter': '.'+filter,
                href:'#',
                'class':'filter',
            }).appendTo(li);
