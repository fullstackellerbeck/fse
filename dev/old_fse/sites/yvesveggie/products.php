		      ...
		      <table width="737" border="0" cellspacing="0" cellpadding="0">
		        ...
		        <tr> 
		            ...
		            <table width="100%" border="0" cellspacing="0" cellpadding="0">
							...
							<?	$rowCount = 1;
									...
									if( $db->numRows( $familyResult ) ) {
										while( list( $familyId, $name ) = $db->fetchRow( $familyResult ) ) {
											$pName = str_replace( " ", "%20", $name );
											$pName = str_replace( "&", urlencode( "&" ), $pName );

											$productResult = $db->query( "select thumb_image from products where family_id = '$familyId' and live = 'Y'" );
											if( $db->numRows( $productResult ) ) {
												list( $thumbImage ) = $db->fetchRow( $productResult );
											}
											echo( $rowCount == 1 ? "<tr>" : "" );	?>
			                <td align="center" valign="top" width="118"> 
			                  	 <a href="products_family.php?family_id=<?= $familyId; ?><?= ($page ? "&page=$page" : "") . ($pId ? "&pId=$pId" : "") . ("&pIdName=$pName"); ?>"><img src="<?= $thumbImage; ?>" width="110" height="110" alt="<?= $name; ?>" border="0" /></a><br />
			                    <a href="products_family.php?family_id=<?= $familyId; ?><?= ($page ? "&page=$page" : "") . ($pId ? "&pId=$pId" : "") . ("&pIdName=$pName"); ?>" class="small"><?= prepStr( $name ); ?></a>
			                </td>
				         	<?	if( $rowCount == 4 ) {
				         				echo( "</tr>" );
				         				$rowCount = 1;
		          ...
		          <?	}	?>
		            ...
		            <p>&nbsp;</p>
		            <p><?= ( $autoHTML == "Y" ? nl2br( strtr( stripslashes( $extraContent ), $trans ) ) : strtr( stripslashes( $extraContent ), $trans ) );	?></p>
		            <p>&nbsp;</p>
		            </td>
		        </tr>
