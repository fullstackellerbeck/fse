		        ...
		        <tr> 
		          ...
		          <td valign="top" align="left"> 
		            <h1><img src="<?= $titleImage; ?>" width="<?= $width; ?>" height="<?= $height; ?>" alt="<?= $pageName; ?>" /></h1>
							...
							<?	$storeResult = $db->query( "select name from stores where province_id = '$province_id' and live = 'Y'" );
									if( $db->numRows( $storeResult ) ) {	?>
										<p><?= prepStr( $copyArr[ "store_search_found" ] );	?></p>
										<p><ul>
									<?	while( list( $storeName ) = $db->fetchRow( $storeResult ) ) {	?>
			            			<li><b><?= prepStr( $storeName ); ?></b></li>
			            <?	}	?>
		            		</ul></p>
		           <?	} else {	?>
		            		<p><?= prepStr( $copyArr[ "store_search_not_found" ] );	?></p>
		           <?	}	?>
