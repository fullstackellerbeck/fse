						...
						while( list( $countryId, $countryName ) = $db->fetchRow( $countryResult ) ) {
							...
							if( $db->numRows( $languageResult ) ) {
								while( list( $languageId, $languageName ) = $db->fetchRow( $languageResult ) ) {
									$languageCountResult = $db->query( "select count( faq_id ) from faq where country_id = '$countryId' and language_id = '$languageId'" );
									...
									if( $languageCount > 0 ) {
										...
										<div class="innercontent" style="width: 555px;">
											...
											<div class="left" style="width: 16%; padding-left: 5px;"><b>Priority</b></div>
											<div class="left" style="width: 13%; padding-left: 5px;"><b>Units</b></div>
											<div class="left" style="width: 13%; padding-left: 5px;"><b>RDI</b></div>
											<div class="spacer"></div>
											<div class="divider"></div>
										
									<?	$displayResult = $db->query( "select attribute_id, name, units, is_rdi, priority ".
											...
											if( $db->numRows( $displayResult ) ) {
												$bgColor = false;
												while( list( $attribId, $name, $units, $is_rdi, $priority ) = $db->fetchRow( $displayResult ) ) {	?>
													<div style="padding-top: 3px; padding-bottom: 0px; background-color: <?= $bgColor ? "#fcfafb" : "#faf7f2"; ?>;" onmouseover="this.style.backgroundColor='#eeeeff';" onmouseout="this.style.backgroundColor='<?= $bgColor ? "#fcfafb" : "#faf7f2"; ?>';">									
														...
														<div class="left" style="padding-left: 5px; width: 16%;"><?= $priority; ?></div>
														<div class="left" style="padding-left: 5px; width: 13%;"><?= $units; ?></div>
														<div class="left" style="padding-left: 5px; width: 13%;"><?= $is_rdi; ?></div>
														<div class="right" style="width: 15%; text-align: right;"><a href="product_nutrition_edit.php?attribute_id=<?= $attribId; ?>">edit</a> &middot; <a href="javascript:void(delItem('product_nutrition_display.php','<?= $attribId; ?>','nutritional_attribs','attribute_id'));">delete</a></div>
