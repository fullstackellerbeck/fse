<?	// objectiveDb database abstraction class
		...
		class objectiveDb {
			...
			function fieldEnumTypes( $table, $fieldName ) {
				if( $this->mySQL ) {
					if( $typeResult = $this->query( "show fields from $table" ) ) {
						while( list( $field, $type, $isNull, $isKey, $default, $extra ) = $this->fetchRow( $typeResult ) ) {
							if( $field == $fieldName ) {		
								if( ereg_replace( "\\(.*", "", $type ) == "enum" )
					        $infoTypes = explode( ",", str_replace( "','", ",", substr( ereg_replace( "\\)$", "", str_replace( "enum(", "", $type ) ), 1, -1 ) ) );
							}	
						}
					}
				}	
