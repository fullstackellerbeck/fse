		...
		if( $continue || $new ) {
			...
			$pageQuery = $db->createInsert( array( "name"           => "'$name'",
	                                           ...
	                                           "p_attribute_id" => "'$p_attribute_id'" ),
																						 ...
																						 "nutritional_compare_attribs" );
	
			$pageResult = $db->query( $pageQuery );
			
			header( "Location: product_compare_nutrition_display.php" );
		}	?>		
...
<html>
	...
	<body>
<?	if( $attribute_id ) {
			$displayResult = $db->query( "select name, units, p_attribute_id, country_id, language_id, priority from nutritional_compare_attribs where attribute_id = '$attribute_id'" );
			if( $db->numRows( $displayResult ) ) {
				list( $name, $units, $pAttributeId, $dbCountryId, $dbLanguageId, $priority ) = $db->fetchRow( $displayResult );
			}
		}	?>
		<form name="bunny" action="product_compare_nutrition_edit.php" method="post">
			...
			<div style="margin: 5px 5px 5px 5px;">
				...
				<div class="contentbox">
					<span class="title"><?= $nav_id ? "Edit" : "Add"; ?> a comparative nutritional attribute</span><br>
					<div class="innercontent">
						<div class="left" style="width: 80%;">
							...
							<div class="left" style="width: 60%;">
								<select name="p_attribute_id" class="adminselect">
									<option value="0"<?= ($pAttributeId == 0 ? ' selected' : ''); ?>>(is a parent item)</option>
							<?	$countryResult = $db->query( "select country_id, name from countries" );
									if( $db->numRows( $countryResult ) ) {
										while( list( $dbCountryId, $countryName ) = $db->fetchRow( $countryResult ) ) {
											...
											if(	 $db->numRows( $languageResult ) ) {
												while( list( $dbLanguageId, $languageName ) = $db->fetchRow( $languageResult ) ) {
													...
													<?	$pAttResult = $db->query( "select attribute_id, name from nutritional_compare_attribs where country_id = '$dbCountryId' and language_id = '$dbLanguageId'" );
															if( $pAttResult && $db->numRows( $pAttResult ) ) {
																while( list( $dbPAttributeId, $dbPName, $dbIsRDI ) = $db->fetchRow( $pAttResult ) ) {	?> 
																	<option value="<?= $dbPAttributeId; ?>"<?= ($dbPAttributeId == $pAttributeId ? ' selected' : ''); ?>><?= $dbPName; ?></option>
														<?	}
															}	?>
						...
						<div class="left" style="width: 20%;">
							...
							<input type="button" name="cancel" value="Cancel" class="adminbutton" onclick="confirmCancel('product_compare_nutrition_display.php');">
						</div>
						<div class="spacer"></div>
					</div>
				</div>
