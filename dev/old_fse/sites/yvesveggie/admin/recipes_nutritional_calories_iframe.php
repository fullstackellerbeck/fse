<?	include( "inc/init.php" );
		...
		if( ($recipe_id || $_SESSION[ "new_recipe_id" ] ) && $continue ) {
			...
			$nutRemoveResult = $db->query( "delete from recipes_nutritional_calories where recipe_id='$recipe_id'" );
			
			for( $i = 0; $i < sizeof( $name ); $i++ ) {
				if( $name[ $i ] != "" ) {
					$nutInsertResult = $db->query( "insert into recipes_nutritional_calories ".
...
<?	}	?>
	...
	<body style="width: 448px; background-color: #f2efe3; margin-top: 0px;">
		...
		<form name="bunny" action="recipes_nutritional_calories_iframe.php" method="post">
			<input type="hidden" name="continue" value="true">
			<input type="hidden" name="recipe_id" value="<?= $recipe_id; ?>">
			
	<?	$bgColor = false;
	
			$nutResult = $db->query( "select calorie_id, name, value from recipes_nutritional_calories where recipe_id = '$recipe_id' order by calorie_id asc" );
			if( $db->numRows( $nutResult ) ) {
				while( list( $calorieId, $name, $vvalue ) = $db->fetchRow( $nutResult ) ) {	?>
					<div style="background-color: <?= $bgColor ? "#faf7f2" : "#f2efe3"; ?>; padding: 1px 10px 1px 10px;" onmouseover="this.style.backgroundColor='#dddddd';" onmouseout="this.style.backgroundColor='<?= $bgColor ? "#faf7f2" : "#f2efe3"; ?>';">
						...
						<div class="left" style="width: 130px;" style="padding-left: 5px;"><input type="text" name="vvalue[]" class="admintext" style="width: 130px; background-color: #ffffff;" value="<?= $vvalue; ?>"></div>
