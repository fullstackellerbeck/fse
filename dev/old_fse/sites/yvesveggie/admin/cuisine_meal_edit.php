<?	include( "inc/init.php" );
		...
		if( $continue || $new ) {
			if( !$tableName ) {
				$tableName = ( $meal_id ? "recipes_meals" : "recipes_cuisine" );
			}
			...
			$pageQuery = $db->createInsert( array( "country_id"  => "'$country_id'",
	                                           ...
	                                           "name"        => "'$name'",
	                                           "live"        => "'$live'" ),
																						 array( ($meal_id ? "meal_id" : "cuisine_id"), ($meal_id ? $meal_id : $cuisine_id) ),
																						 $tableName );
	
			$pageResult = $db->query( $pageQuery );
			header( "Location: cuisine_meal_display.php" );
		}	?>		
...
<html>
	...
	<body>
<?	if( $meal_id ) {
			$query = "select country_id, language_id, name, live from recipes_meals where meal_id = '$meal_id'";
		} elseif( $cuisine_id ) {
			$query = "select country_id, language_id, name, live from recipes_cuisine where cuisine_id = '$cuisine_id'";
		}
		
		if( $query ) {
			$displayResult = $db->query( $query );
			if( $db->numRows( $displayResult ) ) {
				list( $dbCountryId, $dbLanguageId, $name, $live ) = $db->fetchRow( $displayResult );
			}
		}	?>
		<form name="bunny" action="cuisine_meal_edit.php" method="post">
			<input type="hidden" name="<?= $meal_id ? "meal_id" : "cuisine_id"; ?>" value="<?= $meal_id ? $meal_id : $cuisine_id; ?>">
	<?	if( $tableName ) {	?>
	...
	<?	}	?>
			...
			<div style="margin: 5px 5px 5px 5px;">
				...
				<div class="contentbox">
					<span class="title"><?= $meal_id || $cuisine ? "Edit" : "Add"; ?> <?= $meal_id || $tableName == "recipes_meals" ? "a meal type" : "a cuisine type"; ?></span><br>
					<div class="innercontent">
						...
						<div class="left" style="width: 20%;">
							...
							<input type="button" name="cancel" value="Cancel" class="adminbutton" onclick="confirmCancel('cuisine_meal_display.php');">
						</div>
						<div class="spacer"></div>
					</div>
				</div>
