		      ...
		      <table width="737" border="0" cellspacing="0" cellpadding="0">
		        ...
		        <tr> 
		          ...
		          <td valign="top" align="left"> 
		            <h1><a name="top"><img src="<?= $titleImage; ?>" width="<?= $width; ?>" height="<?= $height; ?>" alt="<?= $pageName; ?>" /></a></h1>
		            <p><?= ( $autoHTML == "Y" ? nl2br( prepStr( $content ) ) : prepStr( $content ) );	?></p>
		            <ul>
		         	<?	$faqResult = $db->query( "select faq_id, question ".
		         	                             "  from faq ".
		         	                             " where country_id  = '$sess_country_id' ".
		         	                             "   and language_id = '$sess_language_id' ".
		         	                             "   and live = 'Y' ".
		         	                             " order by priority asc" );
		         			if( $db->numRows( $faqResult ) ) {
		         				while( list( $faqId, $question ) = $db->fetchRow( $faqResult ) ) {	?>
				              <li style="padding-bottom: 4px;"><a href="#q_<?= $faqId; ?>"><?= prepStr( $question ); ?></a></li>
								<?	}
									}	?>
		            </ul>

	         	<?	$faqResult = $db->query( "select faq_id, question, answer from faq where country_id='$sess_country_id' and language_id='$sess_language_id' and live = 'Y' order by faq_id asc" );
	         			if( $db->numRows( $faqResult ) ) {
	         				while( list( $faqId, $question, $answer ) = $db->fetchRow( $faqResult ) ) {	?>
				          	<p>
				            	<strong><a name="q_<?= $faqId; ?>" style="color: #000; text-decoration: none;"><?= prepStr( $question ); ?></a></strong><br>
				             	<?= prepStr( $answer ); ?>
				            </p>
				           	<p class="small" align="right"><a href="#top"><?= prepStr( $copyArr[ "back_to_top" ] ); ?></a> </p>
							<?	}
								}	?>
			          <p>&nbsp;</p>
		        	</td>
